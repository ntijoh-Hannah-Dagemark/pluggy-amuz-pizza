defmodule Pluggy.PizzaController do   # <-- Change 1: Update module name
  ...
  def new(conn), do: send_resp(conn, 200, render("pizzas/new", []))  # <-- Change 5: Template name ("fruits/new" -> "pizzas/new")
  ...
  def create(conn, params) do
    ...
    redirect(conn, "/pizzas")  # <-- Change 9: URL path ("/fruits" -> "/pizzas")
  end

  def update(conn, id, params) do
    Pizza.update(id, params)  # <-- Change 10: Update to Pizza.update
    redirect(conn, "/pizzas")  # <-- Change 11: URL path ("/fruits" -> "/pizzas")
  end

  def destroy(conn, id) do
    Pizza.delete(id)  # <-- Change 12: Update to Pizza.delete
    redirect(conn, "/pizzas")  # <-- Change 13: URL path ("/fruits" -> "/pizzas")
  end
